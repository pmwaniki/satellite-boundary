...
train_dataset=AEDataset(data=train_vae,dir_image='path')
train_loader=DataLoader(train_dataset,batch_size=8,shuffle=True,drop_last=True)
test_dataset=AEDataset(data=test_vae,dir_image='path')
...
class VAE(nn.Module):
    ...
    def __init__(self, z_dim=32, use_cuda=True):
        ...
        self.encoder = Encoder(latent_dim=z_dim*2)
...
lr=0.001
optimizer = Adam({"lr": lr, "betas": (0.90, 0.999)}, {"clip_norm": 1.0})
...
images=iter(test_loader).__next__()[0]
if USE_CUDA: images=images.cuda()
...
for i in range(len(train_simple_dataset)):
    ...
    joblib.dump(recon,
                filename=os.path.join(path_generated,f'Vae-train_{data.iloc[i]["id"]}_{data.iloc[i]["period"]}.joblib'))
...
joblib.dump((test_ids,test_reconstructed),filename=os.path.join(result_dir,"VAE-test.joblib"))

...
configs = {
    ...
    'prop_gaussian':tune.choice([0.01,0.25,0.5,0.75,0.99]),
...
def get_model(config):
    ...
    model=UNet(n_classes=1,n_channels=4+len(add_bands))
    return model
...
def train_fun(model,optimizer,criterion,train_loader,val_loader,grad_scaler,scheduler=None,iteration=0,aug_list=None):
    ...
    for batch_x,batch_y in train_loader:
        ...
        batch_x=add_bands({'image':batch_x})['image']
        batch_x=(batch_x + broadcast_array_offset)/broadcast_array_scale #rescale added bands to (0,1)
...
result = tune.run(
    ...
    resources_per_trial={"cpu": 6, "gpu": 0.45},
    config=configs,
    local_dir=log_dir,
    num_samples=100,
    name=experiment,
...
with torch.no_grad():
    for batch_x in test_loader:
        ...
        batch_x = (batch_x + broadcast_array_offset) / broadcast_array_scale  # rescale added bands to (0,1)
        batch_x = batch_x - 0.5
        probs = best_model(batch_x).sigmoid()
        pred_test.append(probs.squeeze(1).cpu().numpy())

pred_test = np.concatenate(pred_test,axis=0)
# pred_test_long=pred_test.reshape((-1,256))

images=batch_x.cpu().numpy() + 0.5
masks=probs.cpu().numpy()>0.5
n_images=images.shape[0]
...
submission3[['tile_row_column','label']].to_csv(os.path.join(result_dir,f"Submission-{experiment}.csv"),index=False)